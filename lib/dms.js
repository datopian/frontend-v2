...
class DmsModel {
  ...
  async getResourceSchema(resourceId) {
    try {
      ...
      logger.warn({
        message: `Failed to getResourceSchema | ${resourceId} | ${await e.text()}`
      })
      return {}
    }
  }
  ...
  async getResourceViews(resourceId) {
    try{
      ...
      logger.warn({
        message: `Failed to getResourceViews | ${resourceId} | ${await e.text()}`
      })
      return []
    }
  }
  ...
  async getProfile(owner) {
    try {
      ...
      logger.warn({
        message: `Failed to getProfile | ${owner} | ${await e.text()}`
      })
      return {}
    }
  }
