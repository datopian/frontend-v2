...
class DmsModel {
  ...
  async getPackage(name) {
    ...
    // Fetch view for a resource
    for (let i = 0; i < datapackage.resources.length; i++) {
      datapackage.resources[i].views = await this.getResourceViews(datapackage.resources[i].id)
    }
    return datapackage
  }
