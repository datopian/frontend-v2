...
class DmsModel {
  ...
  async getPackage(name) {
    ...
    // Fetch dictionary fields for the resources
    await Promise.all(datapackage.resources.map(async resource => {
      //TODO Check resource type and make
  ...
  async getResourceSchema(resourceId) {
    try{
      ...
      for (let i = 0; i < fields.length; i++) {
        fields[i] = utils.ckanSchemaToTableSchema(fields[i])
      }
      delete response.result.fields
      return {
        fields: fields
      }
    } catch (e) {
      console.warn('Error fetching resource fields', e)
      return []
    }
  }
