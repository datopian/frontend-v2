...
module.exports.authHandler = (type) => (req, res, next) => {
  ...
  authRequest
    .then(({ status, data: flow }) => {
      ...
      flow.methods.oidc.config.action = new URL(
        flow.methods.oidc.config.action
      ).pathname;

      res.locals.sso = flow.methods.oidc.config;
      res.locals.password = flow.methods.password.config;
      res.locals.accountExists = res.locals.sso.fields.find(
        (item) => item.name === "traits.email"
      );
      next();
    })
    .catch((err) => {
