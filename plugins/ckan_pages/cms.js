...
class CmsModel {
  constructor() {
    this.baseUrl = config.get('CKAN_PAGES_URL')
  }

  // returns promise
  async getPost(slug) {
   const url = `${this.baseUrl}/api/3/action/ckanext_pages_show?page=${slug}`
  ...
  async getListOfPosts(size) {
    ...
    const url = `${this.baseUrl}/api/3/action/ckanext_pages_list`
    const res = await fetch(url)
